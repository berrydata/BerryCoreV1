...
library BerryLibrary {
    ...
    */
    function requestData(
        BerryStorage.BerryStorageStruct storage self,
    ...
    */
    function _payReward(BerryStorage.BerryStorageStruct storage self, BerryStorage.Details[5] memory a) internal {
        // update height
        ...
        uint256 i;
        for (i = 0; i < 5; i++) {
            BerryTransfer.doTransfer(self, address(this), a[i].miner, (reward + self.uintVars[keccak256("currentTotalTips")]) / 5);
        }

        //pay the dev-share
        BerryTransfer.doTransfer(self, address(this), self.addressVars[keccak256("_owner")], reward / 4); //The ten there is the devshare

        //update the total supply
        self.uintVars[keccak256("total_supply")] += (reward + reward / 4);
    }
    ...
    */
    function newBlock(BerryStorage.BerryStorageStruct storage self, string memory _nonce, uint256 _requestId) internal {
        ...
        uint256 _topId = BerryGettersLibrary.getTopRequestID(self);
        ...
        if (_topId > 0) {
            ...
            uint256 newRequestId = BerryGettersLibrary.getTopRequestID(self);
    ...
    */
    function proposeOwnership(BerryStorage.BerryStorageStruct storage self, address payable _pendingOwner) internal {
    ...
    */
    function claimOwnership(BerryStorage.BerryStorageStruct storage self) internal {
    ...
    */
    function updateOnDeck(BerryStorage.BerryStorageStruct storage self, uint256 _requestId, uint256 _tip) internal {
        BerryStorage.Request storage _request = self.requestDetails[_requestId];
        uint256 onDeckRequestId = BerryGettersLibrary.getTopRequestID(self);
