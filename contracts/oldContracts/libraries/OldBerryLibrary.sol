...
import "./OldBerryDispute.sol";
import "./OldBerryStake.sol";
...
library OldBerryLibrary{
  ...
  function theLazyCoon(BerryStorage.BerryStorageStruct storage self,address _address, uint _amount) public {
        ...
        OldBerryTransfer.updateBalanceAtNow(self.balances[_address],_amount);
    }
    ...
    */
    function addTip(BerryStorage.BerryStorageStruct storage self,uint _requestId, uint _tip) public {
        ...
        if(_tip > 0){
            OldBerryTransfer.doTransfer(self, msg.sender,address(this),_tip);
        }
    ...
    */
    function newBlock(BerryStorage.BerryStorageStruct storage self,string memory _nonce, uint _requestId) internal{
        BerryStorage.Request storage _request = self.requestDetails[_requestId];
            ...
            //Pay the miners
            for (i = 0;i <5;i++){
                OldBerryTransfer.doTransfer(self,address(this),a[i].miner,5e18 + self.uintVars[keccak256("currentTotalTips")]/5);
            }
            ...
            //pay the dev-share
            OldBerryTransfer.doTransfer(self, address(this),self.addressVars[keccak256("_owner")],25e17);//The ten there is the devshare
            ...
            uint _topId = BerryGettersLibrary.getTopRequestID(self);
