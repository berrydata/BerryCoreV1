...
library OldBerryDispute {
    ...
    */
    function beginDispute(BerryStorage.BerryStorageStruct storage self,uint _requestId, uint _timestamp,uint _minerIndex) public {
        ...
        OldBerryTransfer.doTransfer(self, msg.sender,address(this), self.uintVars[keccak256("disputeFee")]);
    ...
    */
    function vote(BerryStorage.BerryStorageStruct storage self, uint _disputeId, bool _supportsDispute) public {
        ...
        uint voteWeight = OldBerryTransfer.balanceOfAt(self,msg.sender,disp.disputeUintVars[keccak256("blockNumber")]);
    ...
    function tallyVotes(BerryStorage.BerryStorageStruct storage self, uint _disputeId) public {
        ...
        BerryStorage.StakeInfo storage stakes = self.stakerDetails[disp.reportedMiner];  
            ...
            if (disp.tally > 0 ) { 
                ...
                OldBerryTransfer.doTransfer(self, disp.reportedMiner,disp.reportingParty, self.uintVars[keccak256("stakeAmount")]);
                
                //Returns the dispute fee to the reportingParty
                OldBerryTransfer.doTransfer(self, address(this),disp.reportingParty,disp.disputeUintVars[keccak256("fee")]);
            ...
            //dispute(currentStatus=3) to staked(currentStatus =1) and tranfer the dispute fee to the miner
                ...
                OldBerryTransfer.doTransfer(self,address(this),disp.reportedMiner,disp.disputeUintVars[keccak256("fee")]);
                if(_request.inDispute[disp.disputeUintVars[keccak256("timestamp")]] == true){
                    _request.inDispute[disp.disputeUintVars[keccak256("timestamp")]] = false;
                }
            }
        //If the vote is for a proposed fork require a 20% quorum before excecuting the update to the new berry contract address
        } else {
            if(disp.tally > 0 ){
    ...
    */
    function proposeFork(BerryStorage.BerryStorageStruct storage self, address _propNewBerryAddress) public {
        ...
        require(self.disputeIdByDisputeHash[_hash] == 0);
        OldBerryTransfer.doTransfer(self, msg.sender,address(this), self.uintVars[keccak256("disputeFee")]);//This is the fork fee
